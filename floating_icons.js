...
(function () {
    ...
    var animationKeys = ["FloatY", "FloatX", "FloatRot", "FloatScale"];

    function removeAnimations(view, label) {
        if (!isPtr(view)) return false;
        var layer = msgMain(view, "layer");
        if (!isPtr(layer)) return false;
        for (var i = 0; i < animationKeys.length; i++) {
            msgMain(layer, "removeAnimationForKey:", ns(animationKeys[i]));
        }
        say("[Floating Icons] Cleared " + label + ".");
        return true;
    }
    ...
    function cleanup() {
        ...
        for (var i = 0; i < targets.length; i++) {
            if (removeAnimations(targets[i].view, targets[i].label)) cleared++;
        }
        say("[Floating Icons] Cleanup complete: cleared " + cleared + " layer(s).");
        return true;
    }

    // Cyanide calls this before disabling a script and before tearing down QuickJS.
    if (typeof globalThis !== "undefined") {
        globalThis.cleanup = cleanup;
    } else {
        say("[Floating Icons] WARNING: cleanup export is unavailable in this JS runtime.");
    }
