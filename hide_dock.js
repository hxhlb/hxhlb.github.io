...
(() => {
    ...
    function boolValue(v) {
        return !(v === null || v === undefined || v === 0 || v === "0" || v === "0x0" || v === false);
    }
    ...
    function hideView(view, label) {
        ...
        for (var i = 0; i < hiddenTargets.length; i++) {
            if (String(hiddenTargets[i].view) === key) {
                known = true;
                break;
            }
        }
        if (!known) {
            hiddenTargets.push({ view: view, label: label, hidden: boolValue(msgMain(view, "isHidden")) });
        }
    ...
    function cleanup() {
        var restored = 0;
        for (var i = 0; i < hiddenTargets.length; i++) {
            var target = hiddenTargets[i];
            if (!isPtr(target.view)) continue;
            msgMain(target.view, "setHidden:", target.hidden ? 1 : 0);
            msgMain(target.view, "setNeedsLayout");
            restored++;
        }
        log("Hide Dock: cleanup complete; restored " + restored + " view(s).");
    }

    try {
        globalThis.cleanup = cleanup;
    } catch (_) {
        log("Hide Dock: WARNING: cleanup export is unavailable in this JS runtime.");
    }
