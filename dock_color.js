...
(function () {
    ...
    function colorView(view, label, color) {
        ...
        coloredTargets.push({
            view: view,
            label: label,
            backgroundColor: msgMain(view, "backgroundColor"),
            hidden: truthy(msgMain(view, "isHidden"))
        });
    ...
    function cleanup() {
        ...
        say("[DockColor] Cleanup complete: restored " + restored + " dock view(s).");
    }

    try {
        globalThis.cleanup = cleanup;
    } catch (_) {
        say("[DockColor] WARNING: cleanup export is unavailable in this JS runtime.");
    }
