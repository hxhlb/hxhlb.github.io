...
var hiddenSearchPillTargets = [];

function hideSearchPill(view) {
    if (view === "0x0") return false;
    var wasHidden = r_msg2_main(view, "isHidden");
    hiddenSearchPillTargets.push({
        view: view,
        hidden: wasHidden !== "0" && wasHidden !== 0
    });
...
function cleanup() {
    var restored = 0;
    for (var i = 0; i < hiddenSearchPillTargets.length; i++) {
        var target = hiddenSearchPillTargets[i];
        if (target.view === "0x0") continue;
        r_msg2_main(target.view, "setHidden:", target.hidden ? 1 : 0);
        r_msg2_main(target.view, "setNeedsLayout");
        restored++;
    }
    log("[Hide Search Pill] Cleanup complete: restored " + restored + " view(s).");
}

try {
    globalThis.cleanup = cleanup;
} catch (_) {
    log("[Hide Search Pill] WARNING: cleanup export is unavailable in this JS runtime.");
}
